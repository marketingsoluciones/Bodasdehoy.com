      ...
      >
        <Form className="py-6 flex flex-col gap-4 ">
          <div className="flex items-center gap-4 w-full">
            <label className="text-tertiary w-full">
...
export const SelectField: FC<propsSelectField> = (props) => {
  ...
  return (
    <div className="relative">
      <select
        className="text-sm text-gray-300 px-2 py-2 rounded-lg focus:outline-none appearance-none pr-8"
      ...
      <ArrowIcon width={3} className="w-5 h-5 text-primary rotate-90 transform absolute inset-y-0 my-auto right-2" />
    </div>
  );
};
...
const RatingField : FC <propsRatingField>  = (props) => {
    const [field, meta, helpers] = useField({ ...props });
    return (
        <div className="gap-2 grid grid-cols-2 w-80">
